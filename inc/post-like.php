...
function sl_enqueue_scripts() {
    wp_enqueue_script( 'simple-likes-public-js', get_template_directory_uri() . '/assets/js/likes.js', array( 'jquery' ), '0.5', false );
    wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
        'ajaxurl' => admin_url( 'admin-ajax.php' ),
        'like' => __( 'Like', 'sandbox' ),
        'unlike' => __( 'Unlike', 'sandbox' )
    ) );
}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
    ...
    if ( already_liked( $post_id, $is_comment ) ) {
        ...
        $title = __( 'Unlike', 'sandbox' );
        $icon = $icon_full;
    } else {
        $class = '';
        $title = __( 'Like', 'sandbox' );
        $icon = $icon_empty;
    }
    $output = '<a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&post_id=' . $post_id . '&nonce=' . $nonce . '&is_comment=' . $is_comment . '&disabled=true' ) . '" class="sl-button ' . $post_id_class . $class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $icon . $count . '</a>' . $loader . '';
    return $output;
...
function get_liked_icon() {
    ...
    $icon = '<i style="color:red" class="uil uil-heart-alt"></i>';
    return $icon;
} // get_liked_icon()
...
function get_unliked_icon() {
    ...
    $icon = '<i class="uil uil-heart-alt"></i>';
    return $icon;
} // get_unliked_icon()
...
function show_user_likes( $user ) { ?>
    <table class="form-table">
        <tr>
            <th><label for="user_likes"><?php _e( 'You Like:', 'sandbox' ); ?></label></th>
            <td>
                <?php
                ...
                <?php else : ?>
                    <p><?php _e( 'You do not like anything yet.', 'sandbox' ); ?></p>
                <?php
                endif;
                wp_reset_postdata();
                ?>
